...
| **Stylesheets (kept)** | Top-level linked sheets the user opted into `mode: 'file'`, plus every page's inline `<style>` (page-scoped by default, see below) | Flattened `@import` graph, Google imports stripped, `url()` normalised; committed via `tx.addStylesheets` as a `SiteFile` (`type: 'style'`) + `site.runtime.styles` entry scoped to the linking pages. Editable afterwards in the Site panel's Styles section and the code editor. |
...
### Inline `<style>` blocks are page-scoped by default

In the source site a page's inline `<style>` applies to that document only. Importing it into the site-wide rules registry would promote a page-local override (download.html's 42vh swiper height) into the global cascade, where a later import order lets it suppress the shared sheet's rule on **every** page — and re-imports pile the same rules up again. Inline CSS therefore imports in `mode: 'file'` semantics by default (`options.inlineStyleMode`, default `'file'`): each page's inline CSS becomes a page-scoped stylesheet with the synthetic identity `<htmlPath>::inline`, kept after every kept linked sheet so it holds the same override position the browser gives it. Trusted Google CSS2 `@import`s inside it are stripped + installed as fonts. `inlineStyleMode: 'convert'` restores the legacy parse into global style rules (appended last to the page's cascade, origin `<htmlPath>::inline`).

Re-importing a page updates its same-path stylesheet file **in place** (`importedSiteFiles.commitFiles` matches path + type): content and runtime scope are replaced, no `-2` twin is appended.
